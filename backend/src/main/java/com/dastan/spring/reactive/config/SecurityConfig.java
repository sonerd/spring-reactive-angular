...
public class SecurityConfig {
    ...
    private Mono<AuthorizationDecision> currentUserMatchesPath(Mono<Authentication> authentication, AuthorizationContext context) {
        return authentication.map(a -> context.getVariables().get("user").equals(a.getName())).map(AuthorizationDecision::new);
    }
    ...
    public AuthenticationWebFilter loginFilter(ReactiveAuthenticationManager authenticationManager,
        ...
        filter.setServerAuthenticationConverter(exchange -> exchange.getRequest().getBody().cache().next().flatMap(buffer -> {
            try {
        ...
        filter.setAuthenticationSuccessHandler((webFilterExchange, authentication) -> {
            ...
            var data = Map.of("name", user.getUsername(), "roles", AuthorityUtils.authorityListToSet(user.getAuthorities()));
            try {
    ...
    public LogoutWebFilter logoutFilter() {
        ...
        filter.setLogoutHandler((webFilterExchange, authentication) -> webFilterExchange.getExchange().getSession().flatMap(WebSession::invalidate).then());
